...
pub fn createDirectory(implementation: PatchIO.Implementation, parent_dir: PlatformHandle, directory_name: []const u8) PatchIO.PatchIOErrors!PatchIO.PlatformHandle {
    var self = @as(*Self, @ptrCast(@alignCast(implementation.instance_data)));
    _ = self;

    const path_w = windows.sliceToPrefixedFileW(directory_name) catch return PatchIO.PatchIOErrors.OutOfMemory;
    var span = path_w.span();
    ...
    const rc = ntdll.NtCreateFile(
        &file_handle,
        windows.FILE_ADD_SUBDIRECTORY | windows.FILE_TRAVERSE | windows.SYNCHRONIZE, //DesiredAccess
        &attr,
        &file_io,
        null, //AllocationSize
        0, // FileAttributes
        windows.FILE_SHARE_READ | windows.FILE_SHARE_WRITE | windows.FILE_SHARE_DELETE, // ShareAccess
        windows.FILE_CREATE, // CreateDisposition
        windows.FILE_DIRECTORY_FILE, // CreateOptions
        null, //EaBuffer
        0, //EaLength
    );
...
fn closeHandle(implementation: PatchIO.Implementation, handle: PatchIO.PlatformHandle) void {
    _ = implementation;
