...
enable_extensive_logging: bool = false,
lock_workers_to_cores: bool = false,

const StateEnum = enum(u16) {
...
pub fn printJobQueues(self: *ThreadPool) void {
    ...
    while (true) {
        if (threadPtr) |thread| {
            if (thread.run_buffer.count() > 0 or thread.run_queue.canPop())
                std.log.info("Thread [{}] has some work", .{thread.idx});

            // std.log.info("BufferSize: {}, QueueSize: {}", .{thread.run_buffer.count(), thread.run_queue.canPop()});
            threadPtr = thread.next;
        } else {
            break;
        }
    }

    if (self.run_queue.canPop()) {
        std.log.info("MainThreadQueue has work", .{});
    }
}
...
pub fn spawnThreads(self: *ThreadPool) void {
    ...
    var threadNamingBuffer: [128]u8 = undefined;

    while (numSpawned < self.max_threads) : (numSpawned += 1) {
        const spawn_config = std.Thread.SpawnConfig{ .stack_size = self.stack_size };

        const thread = std.Thread.spawn(spawn_config, Thread.run, .{ self, numSpawned }) catch return self.unregister(null);

        // Lock the workers to their core
        if (self.lock_workers_to_cores) {
            var base: u32 = 1;
            var mask = base <<| (numSpawned + 1);

            //TODO: Set thread affinity for linux
            if (builtin.os.tag == std.Target.Os.Tag.windows) {
                _ = SetThreadAffinityMask(thread.getHandle(), mask);
            }
        }
...
pub fn schedule(self: *ThreadPool, batch: Batch) void {
    ...
    if (Thread.current) |thread| {
        if (self.enable_extensive_logging)
            std.log.info("Adding job to run buffer, threadidx: {}", .{thread.idx});

        thread.run_buffer.push(&list) catch thread.run_queue.push(list);
    } else {
        if (self.enable_extensive_logging)
            std.log.info("adding job to run queue", .{});
...
noinline fn notifySlow(self: *ThreadPool, is_waking: bool) void {
    ...
    while (sync.state != @intFromEnum(StateEnum.shutdown)) {
        ...
        sync = @as(Sync, @bitCast(self.sync.tryCompareAndSwap(
            ...
            if (self.enable_extensive_logging) {
                std.log.info("Added notify, idle threads={}", .{sync.idle});
            }

            // We signaled to notify an idle thread
            if (can_wake and sync.idle > 0) {
                if (self.enable_extensive_logging)
                    std.log.info("Triggering idle event", .{});
    ...
    while (true) {
        ...
        if (sync.notified) {
            ...
            sync = @as(Sync, @bitCast(self.sync.tryCompareAndSwap(
                ...
                if (self.enable_extensive_logging and is_idle) {
...
pub noinline fn shutdown(self: *ThreadPool) void {
    ...
    while (sync.state != @intFromEnum(StateEnum.shutdown)) {
        ...
        sync = @as(Sync, @bitCast(self.sync.tryCompareAndSwap(
            ...
            if (self.enable_extensive_logging) {
                std.log.info("Shutting thread pool down", .{});
            }

            if (sync.idle > 0) {
                if (self.enable_extensive_logging) {
                    std.log.info("Marking idle event as shutdown", .{});
                }
...
pub const Thread = struct {
    ...
    fn run(thread_pool: *ThreadPool, thread_idx: usize) void {
        var self = Thread{};
        current = &self;
        ...
        while (true) {
            ...
            if (thread_pool.enable_extensive_logging)
                std.log.info("Woke thread up [{}] ", .{Thread.current.?.idx});
            ...
            while (self.pop(thread_pool)) |result| {
                ...
                if (thread_pool.enable_extensive_logging)
                    std.log.info("Thread [{}] popped some work", .{Thread.current.?.idx});
            ...
            if (thread_pool.enable_extensive_logging)
                std.log.info("No more work to do, thread [{}] is going back to sleep", .{Thread.current.?.idx});
        }
    }
...
const Node = struct {
    ...
    const Queue = struct {
        ...
        fn canPop(noalias self: *Queue) bool {
            var consumer_ref = self.tryAcquireConsumer() catch return false;
            defer self.releaseConsumer(consumer_ref);
